...
func (c *client.Client) GetRemoteFileInfo(ctx context.Context, remoteURL string) (*RemoteFileInfo, error) {
	var result RemoteFileInfo
	encodedURL := url.PathEscape(remoteURL)
	path := fmt.Sprintf("/api/remote-files/%s", encodedURL)
...
// UploadRemoteFile uploads a file from a given URL.
func (c *client.Client) UploadRemoteFile(ctx context.Context, req *RemoteFileUploadRequest) (*RemoteFileUploadResponse, error) {
	var result RemoteFileUploadResponse
	err := c.sendRequest(ctx, "POST", "/api/remote-files/upload", req, &result, nil)
	if err != nil {
		return nil, err
	}
	return &result, nil
